...
class TestSample(TestSampleBase):
    ...
    def test_array_args_and_result(self):
        ...
        s = samplebase.Sample(self.tmp_prefix, args=args, name="smpl")
    ...
    def test_scalar_args_and_result(self):
        args = {"x": 2, "y": 3}
        s = samplebase.Sample(self.tmp_prefix, args=args)
        self.assertFalse(s.done)
    ...
    def test_another_sample_object_same_file(self):
        s1 = samplebase.Sample(self.tmp_prefix, args={"x": 42}, name="datsample")
        s2 = samplebase.Sample(self.tmp_prefix, name="datsample")
...
class TestRun(TestSampleBase):
    ...
    def test_sample_one_point_scalar(self):
        args = {"x": 2, "y": "ypsilon"}
        s = samplebase.Sample(self.tmp_prefix, "samplename", args)
        samplebase.run(TestRun.f, [s])
    ...
    def test_sample_three_points_mixed(self):
        samples = []
        args = {"x": 2, "y": np.array([2., 3.])}
        samples.append(samplebase.Sample(self.tmp_prefix, "samplename1", args))
        args = {"x": 2, "y": 3}
        samples.append(samplebase.Sample(self.tmp_prefix, "samplename2", args))
        args = {"x": np.array([3., 3.]), "y": np.array([2., 2.])}
        samples.append(samplebase.Sample(self.tmp_prefix, "samplename3", args))

        samplebase.run(TestRun.f, samples, n_jobs=3)
